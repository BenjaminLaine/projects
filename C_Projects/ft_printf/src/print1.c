...
/*   Updated: 2020/01/30 22:27:40 by blaine           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_d(t_stf *stf)
{
	...
	if (stf->convert[0] == 'h' && stf->convert[1] == 'h')
		...
		num = (short)(va_arg(stf->args, int));
	else if (stf->convert[0] == 'l' && stf->convert[1] == 'l')
		num = (long long)(va_arg(stf->args, long long));
	else if (stf->convert[0] == 'l')
		num = (long)(va_arg(stf->args, long long));
	else
		num = (int)(va_arg(stf->args, int));
	str = ft_itoa(num);
	if (num == 0 && stf->pres == 0)
		str[0] = '\0';
...
void	print_p(t_stf *stf)
{
	...
	p = (unsigned long)va_arg(stf->args, unsigned long int);
